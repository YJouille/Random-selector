...
function display(idDiv, arrayStudents) {
  ...
  for (let i = 0; i < arrayStudents.length; i++) {
    document.getElementById(idDiv).innerHTML += arrayStudents[i] + "<br>";
  }
}
...
window.addEventListener("load", function () {
  ...
  if (nbvisites == 2) {
    ...
    notSelectedStudentsLocal = localStorage
      .getItem("apprenantNoSelected")
      .split(",");
  } else {
    ...
    if (localStorage.getItem("apprenantSelected")) {
      selectedStudentsLocal = localStorage
        .getItem("apprenantSelected")
        .split(",");
    }
...
document.getElementById("randomBtn").addEventListener("click", function () {
  ...
  if (count > 0) {
    ...
    //Affiche dans la div resultat
    document.getElementById("result").innerHTML =
    notSelectedStudentsLocal[randomIndex];

    //colonne de droite
    //affiche cet élement dans la colonne de droite
    document.getElementById("selected").innerHTML +=
      notSelectedStudentsLocal[randomIndex] + "<br>";
